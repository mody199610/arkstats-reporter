		...
		res.on('data', function (chunk) {
  		...
  		res.on('end', function () {
       		...
       		if (action == 'check') {
       			...
       			self.info.coinbase = "Ark " + jsonObject["version"] + " Build:(" +jsonObject["build"]+")";
				self.info.node = "Ark " + jsonObject["version"] + " Build:(" +jsonObject["build"]+")";
...
Node.prototype.getInfo = function() {
	...
	self.askArk('GET','/api/peers/version','nodeinfo', '', self);
	return true;
}
...
Node.prototype.getStats = function(forced) {
	...
	if (lastFetchAgo >= UPDATE_INTERVAL || forced === true) {
		...
		self.askArk('GET','/api/delegates/forging/status','forging', '', self);
		self.askArk('GET','/api/peers/count','peers', '', self);
		self.getLatestBlock();
		self.getInfo();
