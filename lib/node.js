	...
	var req = http.request(options, function(res) {
		...
		res.on('data', function (chunk) {
  		...
  		res.on('end', function () {
			...
			self.info.node = "Ark " + jsonObject["version"] + " | ArkStats 1.2.0";
...
Node.prototype.startConnection = function() {
	console.info("Initialising connection to Ark Node");
...
Node.prototype.checkConnection = function() {
	...
	if(isConnected) {
		console.success("Connection established with Ark Node");
		this.init();
		this.getVersion();
		return true;
	} else {
		if(this._connection_attempts < MAX_CONNECTION_ATTEMPTS) {
			console.info("Connection attempt with Ark Node", chalk.cyan('#' + this._connection_attempts++), "failed");
...
Node.prototype.startSocketConnection = function() {
	if( !this._socket ){
		console.info("Starting socket connection with ArkStats server");
...
Node.prototype.setupSockets = function() {
	...
	socket.on('open', function open() {
		console.info("The socket connection to the ArkStats server has been opened.");
	...
	.on('ready', function() {
		self._socket = true;
		console.success("The socket connection to the ArkStats server has been established.");
	...
	{
		console.info("ArkStats server has asked for node history");
	...
	.on('node-pong', function(data) {
		...
		console.info("Recieved pong from ArkStats server: " + latency + "ms");
		socket.emit('latency', {
			id: self.id,
			latency: latency
		});
...
Node.prototype.getStats = function(forced) {
	...
	if (lastFetchAgo >= UPDATE_INTERVAL || forced === true) {
		console.stats("Getting stats from Ark Node")
		self.getPending();
...
Node.prototype.sendBlockUpdate = function() {
	...
	console.info("Sending new block to ArkStats server");
...
Node.prototype.sendStatsUpdate = function (force){
	if( this.changed() || force ) {
		console.info("Sending stats to ArkStats server");
...
Node.prototype.ping = function() {
	this._latency = _.now();
	console.info("Pinging ArkStats server");
