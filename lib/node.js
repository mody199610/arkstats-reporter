...
function Node () {
	this.info = {
		...
		protocol: "0.1.0",
		api: "0.1.0",
		port: process.env.LISTENING_PORT || 5555,
		os: os.platform(),
		os_v: os.release(),
		client: pjson.version,
		canUpdateHistory: false,
		proposal: process.env.PROPOSAL || ""
	};
		...
		res.on('data', function (chunk) {
  		...
  		res.on('end', function () {
			...
			self.validateLatestBlock(null, block, misc, self);
       			...
       			self.info.coinbase = "RISE " + jsonObject["version"];
			self.info.node = "RISE " + jsonObject["version"] + " | RiseStats 1.2.0";
			...
			if(self._lastPending !== pending){
				console.info("Sending pending transaction info");
			...
			if(jsonObject["success"]) {
				...
				self.stats.peers = jsonObject["peers"].filter(function(value) { return value.status == "OK" }).length;
				self.stats.badPeers = jsonObject["peers"].filter(function (value) { return value.status != "OK" }).length;
			}
...
Node.prototype.startConnection = function() {
	console.info("Initialising connection to Rise Node");
...
Node.prototype.checkConnection = function() {
	...
	if(isConnected) {
		console.success("Connection established with Rise Node");
		this.init();
		this.getVersion();
		return true;
	} else {
		if(this._connection_attempts < MAX_CONNECTION_ATTEMPTS) {
			console.info("Connection attempt with Rise Node", chalk.cyan('#' + this._connection_attempts++), "failed");
			console.info("Retrying. Please wait - this can take up to 2 minutes!");
...
Node.prototype.startSocketConnection = function() {
	if( !this._socket ){
		console.info("Starting socket connection with RiseStats server");
...
Node.prototype.setupSockets = function() {
	...
	socket.on('open', function open() {
		console.info("The socket connection to the RiseStats server has been opened.");
	...
	.on('ready', function() {
		self._socket = true;
		console.success("The socket connection to the RiseStats server has been established.");
	...
	{
		console.info("RiseStats server has asked for node history");
	...
	.on('node-pong', function(data) {
		...
		console.info("Recieved pong from RiseStats server: " + latency + "ms");
		socket.emit('latency', {
			id: self.id,
			latency: latency
		});
	...
	.on('end', function end() {
		self._socket = false;
		console.error("Socket connection end received");
	})
	...
	.on('close', function () {
		self._socket = false;
		console.error("Socket connection has been closed");
	})
	.on('offline', function () {
		self._socket = false;
	...
	.on('online', function () {
		self._socket = true;
		console.info("Network connection is online");
	})
	.on('reconnect', function () {
		console.info("Socket reconnect attempt started");
	})
	...
	.on('reconnected', function (opts) {
		self._socket = true;
		console.success("Socket reconnected successfully after", opts.duration, 'ms');
	...
	.on('reconnect timeout', function (err, opts) {
		self._socket = false;
		console.error("Socket reconnect atempt took too long:", err.message);
	})
...
Node.prototype.getVersion = function() {
	var self = this;
	self.askArk('GET','/api/peers/version','info', '', self);
}
...
Node.prototype.getLatestBlock = function () {
	var self = this;
	self.askArk('GET','/api/blocks?limit=1','lastblock', '', self);
}
...
Node.prototype.validateLatestBlock = function (error, result, timeString, self) {
	...
	if(block.number == 0) {
		console.error("Got bad block:", chalk.reset.cyan(result));
		return false;
	}

	console.info("Current block height:", this.stats.block.number + "/" + block.number);

	if(this.stats.block.number === block.number) {
		return false;
	} else {
		console.info("New block detected:",block.number);
	}
...
Node.prototype.getStats = function(forced) {
	...
	if (lastFetchAgo >= UPDATE_INTERVAL || forced === true) {
		console.stats("Getting stats from Rise Node")
		self.getPending();
...
Node.prototype.sendBlockUpdate = function() {
	...
	console.info("Sending new block to RiseStats server");
	this.emit('block', this.prepareBlock());
}


Node.prototype.sendPendingUpdate = function(){
	console.stats("Sending pending update");
...
Node.prototype.sendStatsUpdate = function (force){
	if( this.changed() || force ) {
		console.info("Sending stats to RiseStats server");
...
Node.prototype.ping = function() {
	this._latency = _.now();
	console.info("Pinging RiseStats server");
